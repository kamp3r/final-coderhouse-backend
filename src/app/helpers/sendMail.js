const transporterGm = require('../email/mail');
const config = require('../../config/config');
const { loggerInfo, loggerWarn, loggerError } = require('../../config/log4js');

function enviarMailOrdenGenerada(orden) {
  const template = orden.productos
    .map(
      (producto) =>
        `<tr><td><img src="${producto.foto}" width="40px"></td><td>${producto.codigo}</td><td>${producto.nombre}</td><td>${producto.cantidad}</td><td>${producto.precio}</td></tr>`
    )
    .join('');
